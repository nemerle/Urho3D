...
#ifndef LIBLLS_SMALLVECTOR_H
#define LIBLLS_SMALLVECTOR_H
...
#define LIBLLS_HAS_RVALUE_REFERENCES \
  (!defined(LIBLLS_NO_CXX11) && !defined(LIBLLS_NO_RVALUE_REFERENCES))

#define LIBLLS_HAS_VARIADIC_TEMPLATES \
  (!defined(LIBLLS_NO_CXX11) && !defined(LIBLLS_NO_VARIADIC_TEMPLATES))

#if !defined(LIBLLS_NO_CXX11) && !defined(LIBLLS_NO_DELETED_FUNCTION)
# define LIBLLS_DELETED_FUNCTION = delete
#else
# define LIBLLS_DELETED_FUNCTION
#endif

#ifndef LIBLLS_NAMESPACE
# define LIBLLS_NAMESPACE lls
#endif

#ifndef LIBLLS_NO_NAMESPACE
namespace LIBLLS_NAMESPACE {
#endif
...
private:
  ...
  typedef typename std::aligned_storage<sizeof(T), alignof(T)>::type U;
  U FirstEl;
...
#if LIBLLS_HAS_RVALUE_REFERENCES
    for (; I != E; ++I, ++Dest)
      *Dest = ::std::move(*I);
    return Dest;
#else
...
/// template parameter.
template <typename T>
class SmallVectorImpl : public SmallVectorTemplateBase<T, std::is_trivial<T>::value> {
  typedef SmallVectorTemplateBase<T, std::is_trivial<T>::value > SuperClass;

  SmallVectorImpl(const SmallVectorImpl&) LIBLLS_DELETED_FUNCTION;
...
protected:
  ...
  explicit SmallVectorImpl(unsigned N)
    : SmallVectorTemplateBase<T, std::is_trivial<T>::value>(N*sizeof(T)) {
  }
...
namespace std {
  ...
  template<typename T>
  inline void
  swap(LIBLLS_NAMESPACE::SmallVectorImpl<T> &LHS, LIBLLS_NAMESPACE::SmallVectorImpl<T> &RHS) {
    LHS.swap(RHS);
  }
  ...
  template<typename T, unsigned N>
  inline void
  swap(LIBLLS_NAMESPACE::SmallVector<T, N> &LHS, LIBLLS_NAMESPACE::SmallVector<T, N> &RHS) {
    LHS.swap(RHS);
  }
}
